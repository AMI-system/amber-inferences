...
def deployment_data(
    ...
    custom_config = Config(
        connect_timeout=60,  # Time to establish a connection (default is 10s)
        read_timeout=300,  # Time to wait for a response (default is 60s)
    )
    ...
    s3_client = session.client(
        "s3", endpoint_url=credentials["AWS_URL_ENDPOINT"], config=custom_config
    )
    ...
    for country in all_countries:
        ...
        for dep in sorted(all_deps):
            ...
            if include_file_count:
                ...
                counts = count_files(bucket_name, prefix, s3_client)
