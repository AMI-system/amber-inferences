...
json_directory="./keys/panama"
region="pan"
output_base_dir="./data/panama"
...
python 01_print_deployments.py   --subset_countries 'Panama' > ${json_directory}/panama_deployments.txt
deployments=($(grep -oP 'Deployment ID: \Kdep[0-9]+' ${json_directory}/panama_deployments.txt))

echo "Panama deployments: ${deployments[@]}"

for deployment_id in "${deployments[@]}"; do
  echo "Processing keys for deployment: $deployment_id"

  json_file="${json_directory}/${deployment_id}_workload_chunks.json"ß
