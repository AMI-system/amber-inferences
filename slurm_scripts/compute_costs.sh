...
json_directory="./keys/compute_costs"
region="cri"
output_base_dir="/gws/nopw/j04/ceh_generic/kgoldmann/compute_costs"
...
for json_file in ${json_directory}/dep*.json; do
  ...
  mkdir -p "${output_base_dir}/${deployment_id}/compute_resources"
  ...
  sbatch --job-name="${region}_${deployment_id}" \
    ...
    --mem 8G \
    --array=1-$num_chunks \
    --output="./logs/compute_costs/${deployment_id}_batch_%a.out" \
  --export=ALL,\
json_file="$json_file",\
